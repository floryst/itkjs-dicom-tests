...
async function warmup() {
  console.time('warmup');
  await runPipelineBrowser( worker, 'dicom', [], [], []);
  console.timeEnd('warmup');
}

warmup()

async function runCustom(files) {
  console.time('read files');
  const ff = Array.from(files, (file) => {
    return PromiseFileReader.readAsArrayBuffer(file).then((ab) => {
      return {
        name: file.name,
        type: IOTypes.Binary,
        data: ab,
  ...
  const fdescs = await Promise.all(ff);
  console.timeEnd('read files');

  console.time('import');
  const results = await runPipelineBrowser(
    worker,
    'dicom',
    ['import', 'output.json', ...fdescs.map((d) => d.name)],
    // outputs
    [{ path: 'output.json', type: IOTypes.Text }],
    // inputs
    [...fdescs.map((d) => ({ path: d.name, type: IOTypes.Binary, data: new Uint8Array(d.data) }))],
  );
  console.timeEnd('import');
  // console.log(results.outputs[0].data);
  console.log(JSON.parse(results.outputs[0].data));
}
